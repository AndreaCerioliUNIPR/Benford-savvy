...
C (To be optimized for speed and efficiency)
...
C                     set *maxsim1<10**6*
C    (also check the value of ncompmax - exclude matrep ==> see version
C     Streamflow_NOmatrep: for analysis of Streamflow data)
...
C
C GOALS:
C * follow Barabesi et al. (JASA, 2022) and replicate some of the 
C   analyses in Matlab software;
C * follow Barabesi et al. (TEST, 2023) and implement LRT and Score tests
C * extend simulations to new tests developed in Barabesi al (submitted, 2023) 
C   against Benford-Savvy manipulations
C * allow for contamination rate < 1 ==> outlier analaysis: mixture of
C   Benford and contaminant distribution
C
...
C If *itest<0* read (and sort) test statistics computed 
C and saved in previous runs (e.g. under truncation/rounding)
C for computing exact p-values of combined tests
C also check that itest<0 must be combined with iread=0

C
C List of tests currently implemented:
...
C powind(14): KSFR   = Kolmogorov-Smirnov test on the significand fractional part 
C                      (6-dimensional vector output of DKSONE)
C powind(15): VKFR   = Kuiper test on the significand fractional part 
C                      (uses output from DKSONE)
C powind(16): XMAD   = 1st digit MAD test (one-sided)
C
C Combined tests: 
C powcomb(1): X2Q = Combined X2-Q
C powcomb(2): QKS = Combined Q-KS
C powcomb(3): X2QKS = Combined X2-Q-KS
C powcomb(4):X2DIFKS = Combined X2DIF-Q-KS 
C powcomb(5):X2DIF2TKS  = Combined X2DIF2T-Q-KS
C powcomb(6):X2DIFQSCORE= Combined X2DIF-Q-SCOREBIC 
C powcomb(7):X2DIFQXLRT = Combined X2DIF-Q-XLRTBIC
C powcomb(8):X2DIFQVK   = Combined X2DIF-Q-VK 
C powcomb(9):X2DIFKSFR  = Combined X2DIF-KSFR: to be used when X2 accepts
C powcomb(10):X2DIFVKFR = Combined X2DIF-VKFR: to be used when X2 accepts
C
...
C increased dimension for maxsim2 for simulation of test
C statistics under the alternative (isavetest = 1)
C * maxsim2 = 10**6 * 
C  
...
C ntestind=16:  # OF INDIVIDUAL TESTS (see list above)
C ntestcomb=10: # OF COMBINED TESTS (see list above)
C
C
...
C more jittering also when n>500 
C (as with idistr=120,121,122 but note that n<=500 if idistr=12 
C  to keep coherence with previous simulations with n=500)
C
C Rounded Benford distribution: as with idistr=120, but with 
C                 *rounding* instead of truncation according to parameter
C                 *itrunc*  (read from file par_trunc2.txt)
C                  ==> truncation if *itrunc=1*
C                  ==> rounding   if *itrunc=0* 
C
C idistr=120: TRUNCNEW = NEW Truncated Benford distribution - npartrue = 1 
C             (Last-Digit(s) Dirac distribution)  ==> the Benford significand 
C              is truncated at the *6th* decimal place (7 digits) 
C              and the *last* *npar=6* digits are replaced with the fixed 
C              digit value 0 in a *given proportion of cases*
C              (these given proportions are read as input parameters)
C              ==> the resulting significand is jittered in the 10th decimal place 
C                  because of the continuity requirement of routine DKSONE
C
C even more jittering for big n and idistr.eq.120: two options 
C
C Rounded Benford distribution: as for idistr=12, but with 
C                 *rounding* instead of truncation according to parameter
C                 *itrunc*  (read from file par_trunc2.txt)
C                  ==> truncation if *itrunc=1*
C                  ==> rounding   if *itrunc=0* 
C
C Check on the actual number of significant digits 
C                    after truncation/rounding 
C                    (not yet implemented for idistr=12)
C                ==> simulate a new Benford number to be truncated/rounded
C                    if the actual number of significant digits is less than
C                    the expected number
C
C TRUNCNEWREP distribution
C idistr=121: NEW Truncated Benford distribution + REPETITIONS ==> similar 
C             to TRUNCNEW (idistr=120) but with:
C             * fixed proportions of *repeated Benford numbers* in *sorted* 
C               simulated significands (read from input file par_rep.txt)
C             * individual truncation of *sorted* significands through the
C               number of digits (read from input file par_numdigit.txt)
C             + jittering to avoid ties in routine DKSONE
C             N.B: REPETITIONS, which require sorting, are introduced 
C                  *before TRUNCATION* (difference wrt TRUNCNEW)
C
C Introduce the repetition matrix *matrep*:
C             * to be computed in data analysis (idistr=(-1))
C             * to be read as input with TRUNCNEWREPIND distribution 
C               (idistr=122): see file benf_matrep.txt 
C
C TRUNCNEWREPIND distribution
C idistr=122: NEW Truncated Benford distribution + INDIVIDUAL REPETITIONS
C             ==> similar to TRUNCNEW (idistr=120) but with:
C             * fixed proportions of *repeated Benford numbers* in *individual* 
C               simulated significands (read from input file benf_matrep.txt)
C             * individual truncation of significands through the
C               number of digits (read from input file par_numdigitind.txt)
C             + jittering to avoid ties in routine DKSONE
C             N.B: REPETITIONS do NOT require sorting ==> use a loop similar
C             to the loop of TRUNCNEW (difference wrt TRUNCNEWREP)
C
...
c
      ...
      integer*4 iss,mm
      integer*8 iss8,isscheck,isscount
      real*8 sscheck
c define vectors of parameters - npar = max # of parameters, including 
...
c define vectors of simulated data, digits and significand
      real*8 simdata(maxn),s(maxn),sFR(maxn)
      integer*4 d1(maxn),d2(maxn),d12(maxn,2),d1b,d1g,d1dif,
     + ipartrue,ipval,ineg
c define Benford probabilities, frequencies, significand and moments
      real*8 benf1p(9),benf12p(9,10),
     + benf1n(9),benf12n(9,10),benfsig,
     + ee,benfE,benfVAR(9,9),benfVARinv(9,9)
      real*8 dd,dd2
c define test statistics (in single precision for quantile computation)
...
c define (dummy) vector of simulated statistics for null DF + percentile
...
c             ==> asymptotic percentiles are used for *individual tests* only
     ...
     + pvalXMADas
c define dummy array of null percentiles ==> to be used (in double precision)
c in power computation: arrays for individual + combined tests
      real*8 pind(nqprop,ntestind)
c define vectors of power-size estimates (exact + asymptotic)
...
c define vectors for checking parameter/digit values      
     ...
     + checksigt(9,npar),checkdigt(9,npar)
      ...
      integer*4 itrunc   
      integer*4 irep(maxn)
      real*8 avnrep,avndist
      real*8 sd
c compute and save the number of digits (with real data)     
      integer*4 ndigit(maxn),nd
c define vectors for truncation/rounding in TRUNCNEWREP distribution
      integer*4 numdigit(maxn),numd
!      integer*4 ipermdigit(maxn)
c define vectors for check of digit truncation/rounding 
c             idistr=12:  npar=6 digits x npar=6 parameters 
c             idistr=120: npar=6 digits x npartrue=1 parameter 
c             idistr=121: npar=6 digits x npartrue=1 parameter 
c             idistr=122: npar=6 digits x npartrue=1 parameter 
      real*8 avntrunc12(npar,npar),avntrunc120(npar),avntrunc121(npar),
     * avntrunc122(npar)
      real*8 sumntrunc12,sumntrunc120,sumntrunc121,sumntrunc122,avnumd
c define repetition matrix to be computed / read as input)
      integer*2 matrep(maxn,maxn)
      integer*4 ii,numrep,numreptot,nunique,iunique(maxn)
c quantities for the moments of XMAD = MAD test on the first digit
      real*8 xpi,Rmat(9,9),Rhomat(9,9),Dvec(9),rhoij,rij,EMAD,VARMAD,
     + EMADn,VARMADn,XMADn     
c      
      EXTERNAL BENFCDF
      EXTERNAL BENFCDFFR      
...
c      isavetest=isavesig
      ...
      if(itest.lt.0.and.iread.eq.1) then
        write(*,*) "Error in *itest* and *iread*"
        go to 9999
      end if        
...
c    (Check ncompmax and memory requirement with very big data ==> see version
c     Streamflow_NOmatrep)
...
c      
c Moments + asymptotic percentiles for XMAD = MAD test on the first digits 
c (from Cerqueti and Lupi, TEST, 2023, Eq. (12))
c Compute the asymptotic percentiles of XMAD instead of 
c     XMADn = standardized MAD = (sqrt(n)*MAD - EMAD)/sqrt(VARMAD)
c     ==> for comparison with the exact percentiles of XMAD 
c     ==> XMAD>0: one sided test with size qprop2(j)
c         as in Cerqueti and Lupi, TEST, 2023, Eq. (14)  
      xpi=3.14159265358979d0
      EMAD=0.d0
      do j=1,9
        Dvec(j)=dsqrt(benf1p(j)*(1.d0-benf1p(j)))
        EMAD=EMAD+Dvec(j)
        do jj=1,9
          if(jj.eq.j) then
            Rhomat(j,jj)=1.d0
          else
            rhoij=benf1p(j)*benf1p(jj)
            rhoij=rhoij/((1-benf1p(j))*(1-benf1p(jj)))
            Rhomat(j,jj)=-1.d0*dsqrt(rhoij)
          end if        
        end do
      end do
      EMAD=EMAD*dsqrt(2.d0/(xpi*9.d0*9.d0))
      EMADn=EMAD/dsqrt(fn)
      do j=1,9
        do jj=1,9
          rij=Rhomat(j,jj)*dasin(Rhomat(j,jj))
     +     +dsqrt(1.d0-Rhomat(j,jj)*Rhomat(j,jj))
          rij=rij*2.d0/xpi-2.d0/xpi
          Rmat(j,jj)=rij        
        end do
      end do
      VARMAD=0.d0
      do j=1,9
        do jj=1,9
          VARMAD=VARMAD+Dvec(j)*Rmat(j,jj)*Dvec(jj)
        end do
      end do
      VARMAD=VARMAD/(9.d0*9.d0)
      VARMADn=VARMAD/fn
      do j=1,nqprop
        qq=qprop2(j)
        qqq=dnorin(qq)
        percas(j,16)=qqq*dsqrt(VARMADn)+EMADn
      end do
C    
C IF *iread=\1*:
...
500   format(2f12.6)
      IF(iread.eq.1) THEN
        ...
        close(65)
        open(unit=66,file='benfXMAD_perc.txt')
        read(66,500) (percind(j,16),qprop(j),j=1,nqprop)
        close(66)
c       
...
c increase output if idistr=121 and idistr=122
!303   format(f18.15,i6,i2,2x,f18.15,2x,f18.15)
!303   format(f18.15,i3,2x,f18.15,2x,f18.15)
c new format 303 (also save some memory space)
303   format(f12.9,i3,1x,f12.9,1x,f12.9)
304   format(f18.15,i3,2x,f18.15,2x,f18.15,2x,f18.15)
c
...
c Open files for saving test statistics
      ...
      OPEN(unit=46, file='benfXMAD.txt')            
c    
...
c
C
C itest<0 ==> skip null simulation and read test statistics 
C               computed and saved in previous runs (e.g. under truncation/rounding)
C               for computing exact p-values of combined tests    
C               NB: This step could be improved
      if(itest.lt.0) goto 999       
C        
C
...
c j=1,...,6 output values in vector KSFR(j) from IMSL routine
...
c Kuiper test on the significand *fractional part* (based on output of DKSONE)
        ...
        write(45,30) VKFR
c        
c XMAD (could be incorporated into X2ONE)
        call XMADONE(jsimul,n,maxn,d1,benf1p,XMAD)
        write(46,30) XMAD        
c
...
C
C
C itest.lt.0 ==> read test statistics computed and saved 
C               in previous runs 
C               NB: This step could be improved by avoiding read+write
C               NB: jsimul and xjsimul not used in this case 
C                  (dummy arguments to replicate formats 40-44)
999   continue
c Open files for saving test statistics
      OPEN(unit=31, file='benfX2.txt')
      OPEN(unit=32, file='benfX2_2d.txt')
      ...
      OPEN(unit=46, file='benfXMAD.txt')            
c    
c Open files for reading test statistics: not all are used ==> compute
c some test statistics from previous input
      OPEN(unit=331, file='simX2.txt')
      OPEN(unit=332, file='simX2_2d.txt')
      OPEN(unit=333, file='simQ.txt')
      OPEN(unit=334, file='simKS.txt')
!      OPEN(unit=335, file='simX2DIF.txt')
!      OPEN(unit=336, file='simX2DIF2T.txt')
      OPEN(unit=337, file='simSCORE1.txt')
      OPEN(unit=338, file='simSCORE2.txt')
      OPEN(unit=339, file='simSCOREBIC.txt')
      OPEN(unit=340, file='simXLRT1.txt')
      OPEN(unit=341, file='simXLRT2.txt')
      OPEN(unit=342, file='simXLRTBIC.txt')      
!      OPEN(unit=343, file='simVK.txt')      
      OPEN(unit=344, file='simKSFR.txt')
!      OPEN(unit=345, file='simVKFR.txt') 
      OPEN(unit=346, file='simXMAD.txt')  
C START OF READING LOOP OVER *nsim1* SIMULATIONS
      do jj=1,nsim1
        mm=mod(jj,loopn)
        if(mm.eq.0) write(*,*) "Reading sim # ",jj
C READ AND SAVE TEST STATISTICS ==> SAVE TO FILE IN ORDER TO REDUCE
C MEMORY REQUIREMENTS (only one dummy vector will be required for computing
C null percentiles) 
C Also compute summary statistics + correlation for X2, Q and X2DIF-X2DIF2T
c DOUBLE PRECISION IS USED FOR COMPUTING - SINGLE PRECISION FOR SAVING
...
c
c Pearson X2 on first-two digits
        read(332,40) X2_2d,jsimul
        write(32,30) X2_2d
c
c Hotelling Sum-invariance test Q
        read(333,40) Q,jsimul
        write(33,30) Q 
        X2Qmean(2)=X2Qmean(2)+Q/fn1
        corX2Q(2,2)=corX2Q(2,2)+Q*Q/fn1
        corX2Q(1,2)=corX2Q(1,2)+X2*Q/fn1
...
c        
c SCORE + LR TESTS 
        read(337,40) SCORE1,jsimul
        read(338,40) SCORE2,jsimul
        read(339,41) SCOREBIC,ncomps,jsimul
        avencompscoreb=avencompscoreb+dfloat(ncomps)/fn1
        varncompscoreb=varncompscoreb+dfloat(ncomps*ncomps)/fn1
        if(ncomps.eq.ncompmax) maxncompscoreb=maxncompscoreb+1.d0/fn1
        write(37,30) SCORE1
        ...
        write(39,30) SCOREBIC
        write(390,20) ncomps
        read(340,40) XLRT1,jsimul
        read(341,40) XLRT2,jsimul
        read(342,41) XLRTBIC,ncompx,jsimul
        avencompxlrtb=avencompxlrtb+dfloat(ncompx)/fn1
        varncompxlrtb=varncompxlrtb+dfloat(ncompx*ncompx)/fn1
        if(ncompx.eq.ncompmax) maxncompxlrtb=maxncompxlrtb+1.d0/fn1
        write(40,30) XLRT1
...
c Kolmogorov-Smirnov test on the significand *fractional part*
c j=1,...,6 output values in vector KSFR(j) from IMSL routine
        read(344,44) (KSFR(j),j=1,6),jsimul
        write(44,34) (KSFR(j),j=1,6)
c        
c Kuiper test on the significand *fractional part* (based on output of DKSONE)
        VKFR=KSFR(2)+KSFR(3)
...
c        
c XMAD (could be incorporated into X2ONE)
        read(346,43) XMAD,xjsimul,jsimul        
        write(46,30) XMAD        
c
C END OF READING LOOP OVER *nsim1* SIMULATIONS 
      end do 
      CLOSE(331)
      CLOSE(332)
      CLOSE(333)
...
C
      if(itest.ge.0) then   
        ...
        write(*,*) "END OF READING TEST STATISTICS"
      end if
c
...
605   format("Kuiper test VK on the significand *fractional part*",
     + 8x,"# sim =",i9,8x,"n =",i6)
      close(65) 
c
c MAD test on the first digit: exact percentiles for XMAD instead of 
c     XMADn = standardized MAD = (sqrt(n)*MAD - EMAD)/sqrt(VARMAD)
c     XMAD>0: one sided test with size qprop2(j)
c     as in Cerqueti and Lupi, TEST, 2023, Eq. (14)  
      write(*,*) "MAD TEST ON THE FIRST DIGIT"
      OPEN(unit=46, file='benfXMAD.txt')
      do jj=1,nsim1
        read(46,30) XMADs
        benfarray(jj)=XMADs
      ...
      CLOSE(46)
      CALL SVRGP (nsim1, benfarray, benfarray, ipermstat)
      OPEN(unit=146,file='benfXMADsort.txt')
      OPEN(unit=246,file='benfXMADperm.txt')      
      do jj=1,nsim1
        write(146,30) benfarray(nsim1-jj+1)
        write(246,20) ipermstat(nsim1-jj+1)
      end do
      CLOSE(146)
      CLOSE(246)
      CALL ORDST (nsim1,benfarray,nqprop,0,iperc,pp, nmiss)
      ...
      open(unit=66,file='benfXMAD_perc.txt')
      write(66,500) (percind(j,16),qprop(j),j=1,nqprop)
      write(66,*)
      write(66,606) nsim1,n
606   format("MAD test on the first digit",8x,"# sim =",i9,8x,"n =",i6)
      close(66)             
c 
c
C
...
c * if idistr=-1 compute and save the number of  
c                     significant digits of each observation before jittering 
c                     with routine COUNTDIGIT (use option b) for approximation)
c                     (up to npar=6 significant digits) 
c * compute the repetition matrix *matrep* ==> to be read as 
c             input with idistr=122 (TRUNCNEWREPIND distribution)
c             computation of matrep is based on *simdata* ==> if repetitions
c             are required on significand, the observed significands
c             must be given as input data
c             
        if(idistr.eq.(-1)) then
          nsim2=1
...
!1         format(f30.10) 
!3         format(f30.8,i8)       
          open(unit=13,file='benf_data_floor.txt')
          open(unit=14,file='benf_digits.txt')
          open(unit=15,file='benf_matrep.txt')
          open(unit=16,file='benf_unique.txt')          
          numreptot=0
          do i=1,n
            do ii=1,n
              matrep(i,ii)=0
          ...
          do i=1,n
            read(11,*) simdata(i)
!            read(11,1) simdata(i)
            write(13,*) simdata(i),floor(simdata(i))
!            write(13,3) simdata(i),floor(simdata(i))
...
c compute and save *nd = number of digits* before jittering
c                     if nd>npar ==> nd=npar  (npar=6)           
            sd=simdata(i)
            call countdigit(sd,nd,i,npar,idistr)
            ndigit(i)=nd
            write(14,*) simdata(i),ndigit(i)
c compute the repetition matrix *matrep* before jittering
c             equality up to 10^-9 ==> see format 1
c             N.B: computation based on simdata ==> input data
            do ii=1,i
              if((simdata(ii)-simdata(i)).eq.0.d0) then
                matrep(ii,i)=1
                matrep(i,ii)=1
                if(i.ne.ii) numreptot=numreptot+1
              else 
                matrep(ii,i)=0
                matrep(i,ii)=0
              end if
...
c save the repetition matrix *matrep* and compute *nunique* 
c             (to be improved)
          nunique=0 
          do ii=1,n
            numrep=0
            ...
            do i=1,n
              if(ii.ne.i) then
                if(matrep(ii,i).eq.1) numrep=numrep+1
              end if
            end do
            if(numrep.eq.0) then
              nunique=nunique+1
          ...
          do ii=1,n
            write(15,*) (matrep(ii,i), i=1,n)
            write(16,*) ii,iunique(ii)
          end do          
c Perform jittering in the data only if *ijit.gt.0*
c NEW LOOP FOR JITTERING ==> to allow computation of *matrep*
c             before jittering (see previous loop)
c             Also invert the loop with if statement         
          if(ijit.gt.0) then
            do i=1,n
              rng=DRNUNF()                           
...
c                istab=2: Kanter algorithm for positive stable distrib. adapted by Barabesi; 
c                         uses alpha only
...
c Rounded Benford distribution if *itrunc=0*
      IF(idistr.eq.12) THEN
        ...
        OPEN(unit=11,file='par_trunc2.txt')
        read(11,*) itrunc
        CLOSE(11)
        if(itrunc.ne.0.and.itrunc.ne.1) then
          write(*,*) "Error: invalid value of *itrunc*"
          go to 9999
        end if
c if *idig1=1* 1st digit is Benford ==> *NO* for this distr. 
        if(itrunc.eq.1) then       
          ...
          open(unit=73,file='powX2Q_summary_ROUND.txt')
          open(unit=75,file='powX2DIF_summary_ROUND.txt')
          open(unit=77,file='powX2DIF2T_summary_ROUND.txt')
          open(unit=101, file='signif_ROUND1.txt') 
          open(unit=102, file='signif_ROUND2.txt') 
          open(unit=103, file='signif_ROUND3.txt') 
          open(unit=104, file='signif_ROUND4.txt') 
          open(unit=105, file='signif_ROUND5.txt') 
          open(unit=106, file='signif_ROUND6.txt')                          
        end if 
        do j=1,npar
          do jj=1,npar
            avntrunc12(j,jj)=0.d0
          end do
        end do       
      END IF    
c idistr =120: NEW Truncated Benford: TRUNCNEWREP = Last-Digit(s) Dirac  
c              distribution with fixed digit 0 in a given proportion of cases
c              (read the given proportions of cases in which the last
c               npar=6 digit values are replaced by 0) ==> npartrue=1
...
c if *idig1=1* 1st digit is Benford ==> *NO* for this distr.
          if(itrunc.eq.1) then
            ...
            open(unit=73,file='powX2Q_summary_ROUNDNEW.txt')
            open(unit=75,file='powX2DIF_summary_ROUNDNEW.txt')
            open(unit=77,file='powX2DIF2T_summary_ROUNDNEW.txt')
            open(unit=101, file='signif_ROUNDNEW.txt')         
          end if
        do j=1,npar
            avntrunc120(j)=0.d0
        end do                 
      END IF
c TRUNCNEWREP distribution if idistr=121 ==> similar to 
c             TRUNCNEW (idistr=120) but with REPETITIONS
c             Read:
c             *individual truncation* of sorted significands from file
c              par_numdigit.txt  ==> refer to observed *sorted significands*
c             *repetitions* of sorted significands from file 
c              par_rep.txt ==> refer to observed *sorted significands*
c             (read also parameter itrunc from file par_trunc2.txt)
      IF(idistr.eq.121) THEN
        npartrue=1
        ntrunc(j)=0
        OPEN(unit=11,file='par_numdigit.txt')
        open(unit=12,file='par_rep.txt')
        avnrep=0.d0
        do i=1,n
          read(11,*) numdigit(i)
          read(12,*) irep(i)
          if(irep(i).eq.1) avnrep=avnrep+1.d0
          ntrunc(numdigit(i))=ntrunc(numdigit(i))+1
        end do
        CLOSE(11)
        close(12)
        cumtrunc(1)=ntrunc(1)        
...
c if *idig1=1* 1st digit is Benford ==> *NO* for this distr.
        if(itrunc.eq.1) then
          open(unit=73,file='powX2Q_summary_TRUNCNEWREP.txt')
          open(unit=75,file='powX2DIF_summary_TRUNCNEWREP.txt')
          open(unit=77,file='powX2DIF2T_summary_TRUNCNEWREP.txt')
          open(unit=101, file='signif_TRUNCNEWREP.txt') 
        else
          open(unit=73,file='powX2Q_summary_ROUNDNEWREP.txt')
          open(unit=75,file='powX2DIF_summary_ROUNDNEWREP.txt')
          open(unit=77,file='powX2DIF2T_summary_ROUNDNEWREP.txt')
          open(unit=101, file='signif_ROUNDNEWREP.txt')         
        end if
        do j=1,npar
            avntrunc121(j)=0.d0
        end do 
c       
        open(unit=121,file='rnb.txt')
      END IF
c TRUNCNEWREPIND distribution if idistr=122 ==> similar to 
c             TRUNCNEW (idistr=120) and TRUNCNEWREP (idistr=121) 
c             but with INDIVIDUAL REPETITIONS
c             Read:
c             *individual truncation* of unsorted significands from file
c              par_numdigitind.txt  ==> refer to observed unsorted 
c             *individual significands* 
c              ==> use the same file *numdigit* as with idistr=121 but here
c               significands are unsorted: see also file benf_digits.txt
c                                          with idistr=-1
c             ==> use the same file *irep* as with idistr=121 but here
c                 as a dummy vector for simulation of new significands
c             *repetitions* of unsorted individual significands from file 
c              par_matrep.txt ==> refer to matrix *matrep* 
c              computed with idistr=-1 (same as file benf_matrep.txt)
c             (read also parameter itrunc from file par_trunc2.txt)
      IF(idistr.eq.122) THEN
        npartrue=1
        ntrunc(j)=0
        avnrep=0.d0
        avnumd=0.d0
        OPEN(unit=11,file='par_numdigitind.txt')
        open(unit=12,file='par_matrep.txt')
        do i=1,n
          read(11,*) numdigit(i)
          read(12,*) (matrep(i,ii),ii=1,n)
          irep(i)=0
          ntrunc(numdigit(i))=ntrunc(numdigit(i))+1
        end do
        CLOSE(11)
...
c if *idig1=1* 1st digit is Benford ==> *NO* for this distr.
        if(itrunc.eq.1) then
          open(unit=73,file='powX2Q_summary_TRUNCNEWREPIND.txt')
          open(unit=75,file='powX2DIF_summary_TRUNCNEWREPIND.txt')
          open(unit=77,file='powX2DIF2T_summary_TRUNCNEWREPIND.txt')
          open(unit=101, file='signif_TRUNCNEWREPIND.txt') 
        else
          open(unit=73,file='powX2Q_summary_ROUNDNEWREPIND.txt')
          open(unit=75,file='powX2DIF_summary_ROUNDNEWREPIND.txt')
          open(unit=77,file='powX2DIF2T_summary_ROUNDNEWREPIND.txt')
          open(unit=101, file='signif_ROUNDNEWREPIND.txt')         
        end if
        do j=1,npar
            avntrunc122(j)=0.d0
        end do             
...
!     +     sngl(sumabsparNNTS)
!        end if       
c Check the parameter constraint with tolerance to avoid 
...
c new file names for test statistics if itest<0
      if((idistr.ge.0).and.(isavetest.eq.1).and.(itest.ge.0)) then
        ...
        OPEN(unit=560, file='simXMAD.txt')
c save exact p-values for the tests to be combined
        OPEN(unit=201, file='sim_Pval.txt')
        write(201,*) 'pvalX2       ','pvalQ        ','pvalKS       ',
     + 'pvalX2DIF    ','pvalX2DIF2T  ','pvalSCOREBIC ','pvalXLRTBIC  ',
     + 'pvalVK       ','pvalKSFR     ','pvalVKFR     ','pvalXMAD     '
      else if((idistr.ge.0).and.(isavetest.eq.1).and.(itest.lt.0)) then
        OPEN(unit=410, file='simX2r.txt')
        OPEN(unit=420, file='simX2_2dr.txt')
        OPEN(unit=430, file='simQr.txt')
        OPEN(unit=440, file='simKSr.txt')
        OPEN(unit=450, file='simX2DIFr.txt')
        OPEN(unit=460, file='simX2DIF2Tr.txt')
        OPEN(unit=470, file='simSCORE1r.txt')
        OPEN(unit=480, file='simSCORE2r.txt')
        OPEN(unit=490, file='simSCOREBICr.txt')
        OPEN(unit=500, file='simXLRT1r.txt')
        OPEN(unit=510, file='simXLRT2r.txt')
        OPEN(unit=520, file='simXLRTBICr.txt')
        OPEN(unit=530, file='simVKr.txt')
        OPEN(unit=540, file='simKSFRr.txt')
        OPEN(unit=550, file='simVKFRr.txt')
        OPEN(unit=560, file='simXMADr.txt')
c save exact p-values for the tests to be combined
        OPEN(unit=201, file='sim_Pvalr.txt')
        write(201,*) 'pvalX2       ','pvalQ        ','pvalKS       ',
     + 'pvalX2DIF    ','pvalX2DIF2T  ','pvalSCOREBIC ','pvalXLRTBIC  ',
     + 'pvalVK       ','pvalKSFR     ','pvalVKFR     ','pvalXMAD     '
      end if     
...
c For X2_2d + SCORE1 + SCORE2 + XLRT1 + XLRT2 the exact p-value is not (yet) computed
      if(idistr.lt.0) then
        ...
        OPEN(unit=66, file='compXMAD.txt')        
      end if
c
...
c more jittering also when n>500
     ...
     +           " in pow sim #",jj         
              ...
              iss8=floor(ss)      
c Rounded Benford distribution if *itrunc=0* 
c                NB: *iss8=10**(6-j+1)*  if  iss8=10**(7-j+1)  after rounding             
              if(itrunc.eq.0) then
                ss=ss-dfloat(iss8)
                if(ss.ge.0.5d0) iss8=iss8+1                
                if(iss8.eq.(10**(7-j+1))) iss8=10**(6-j+1)             
              end if              
              ss=(dfloat(iss8))*(10.d0**(-6+j-1))    
              do ij=1,j
                ss=ss+partrue*10.d0**(-7+ij-1)              
              end do
c check on actual truncation/rounding before jittering
c             npar=6 digits x npar=6 parameters 
c             use option a) for approximation in countdigit
              call COUNTDIGIT(ss,nd,i,npar,idistr)
              avntrunc12(nd,j)=avntrunc12(nd,j)+1.d0/fn2
c             
              rng=DRNUNF()
c more jittering also when n=200
c more jittering also when n>500 
...
c simulate a new Benford number to be truncated/rounded
c                    if the actual number of significant digits is less than
c                    the expected number ==> see label 12000 below
c TRUNCNEWREP distribution if idistr=121 ==> simulate significand
c             then go to label 13000: skip all the subsequent computations
c             (truncation performed outside the loop over sample elements)
            if(idistr.eq.120.or.idistr.eq.121) then 
              partrue=0.d0
12000         continue              
              rnb=DRNUNF()
              if(rnb.le.0.d0.or.rnb.ge.1.d0) then
     ...
     +           " in pow sim #",jj         
              ...
              ssb=10.d0**rnb
              if(isavesig.eq.1) write(121,121) rnb
121           format(f12.10)              
c if idistr=121 *skip computations* 
c             + initialize permutation vector *ipermdigit* (NOT necessary)
              if(idistr.eq.121) then
                s(i)=ssb
!                ipermdigit(i)=i
                goto 13000
              end if
c continue computations only if idistr=120
              ss=ssb*(10.d0**(6-j+1))      
              iss8=floor(ss)
              ss=(dfloat(iss8))*(10.d0**(-6+j-1)) 
c NO check required with 1 significant digit                   
              if(i.le.cumtrunc(1)) then                                     
                ij=6
                iss8=floor(ss)       
c Rounded Benford distribution if *itrunc=0*
c                NB: *iss8=1*  if  iss8=10**1  after rounding               
                if(itrunc.eq.0) then
                  ss=ss-dfloat(iss8)
                  if(ss.ge.0.5d0) iss8=iss8+1
                  if(iss8.eq.(10**(7-ij))) iss8=10**(7-ij-1)
                end if                       
...
c Rounded Benford distribution if *itrunc=0*              
c                NB: *iss8=10*  if  iss8=10**2  after rounding 
                if(itrunc.eq.0) then
                  ss=ss*10.d0-dfloat(iss8)
                  if(ss.ge.0.5d0) iss8=iss8+1
                  if(iss8.eq.(10**(7-ij))) iss8=10**(7-ij-1)
                end if                                      
                ss=(dfloat(iss8)*(10.d0**(-1)))+
     +           partrue*10.d0**(-7+ij-1)
c simulate a new Benford number if the actual number of 
c                    significant digits is < 2 ==> go to label 12000 above 
!                isscheck=floor(ss)
!                sscheck=ss-dfloat(isscheck)
!                if(sscheck.lt.tol) then
!                  isscount=isscount+1
!                  goto 12000
!                end if
c use COUNTDIGIT for checking 
c             use option a) for approximation
                call COUNTDIGIT(ss,nd,i,npar,idistr)                           
                if(nd.lt.2) then
                  isscount=isscount+1
                  goto 12000
...
c Rounded Benford distribution if *itrunc=0*
c                NB: *iss8=100*  if  iss8=10**3  after rounding              
                if(itrunc.eq.0) then
                  ss=ss*10.d0**2-dfloat(iss8)
                  if(ss.ge.0.5d0) iss8=iss8+1
                  if(iss8.eq.(10**(7-ij))) iss8=10**(7-ij-1)                  
                end if                                       
     ...
     +           partrue*10.d0**(-7+ij-1)
c simulate a new Benford number if the actual number of 
c                    significant digits is < 3 ==> go to label 12000 above        
!                isscheck=floor(ss*10.d0**1)
!                sscheck=ss-dfloat(isscheck*10.d0**(-1))
!                if(sscheck.lt.tol) then
!                  isscount=isscount+1
!                  goto 12000
!                end if
...
c Rounded Benford distribution if *itrunc=0*              
c                NB: *iss8=1000*  if  iss8=10**4  after rounding              
                if(itrunc.eq.0) then
                  ss=ss*10.d0**3-dfloat(iss8)         
                  if(ss.ge.0.5d0) iss8=iss8+1
                  if(iss8.eq.(10**(7-ij))) iss8=10**(7-ij-1)
                end if                                   
     ...
     +           partrue*10.d0**(-7+ij-1)
c simulate a new Benford number if the actual number of 
c                    significant digits is < 4 ==> go to label 12000 above       
!                isscheck=floor(ss*10.d0**2)
!                sscheck=ss-dfloat(isscheck*10.d0**(-2))
!                if(sscheck.lt.tol) then
!                  isscount=isscount+1
!                  goto 12000
!                end if   
...
c Rounded Benford distribution if *itrunc=0*              
c                NB: *iss8=10000*  if  iss8=10**5  after rounding              
                if(itrunc.eq.0) then
                  ss=ss*10.d0**4-dfloat(iss8)                   
                  if(ss.ge.0.5d0) iss8=iss8+1
                  if(iss8.eq.(10**(7-ij))) iss8=10**(7-ij-1)
                end if                              
     ...
     +           partrue*10.d0**(-7+ij-1)
c simulate a new Benford number if the actual number of 
c                    significant digits is < 5 ==> go to label 12000 above           
!                isscheck=floor(ss*10.d0**3)
!                sscheck=ss-dfloat(isscheck*10.d0**(-3))   
!                if(sscheck.lt.tol) then
!                  isscount=isscount+1
!                  goto 12000
!                end if 
...
c Rounded Benford distribution if *itrunc=0*              
c                NB: *iss8=100000*  if  iss8=10**6  after rounding              
                if(itrunc.eq.0) then
                  ss=ss*10.d0**5-dfloat(iss8) 
                  if(ss.ge.0.5d0) iss8=iss8+1
                  if(iss8.eq.(10**(7-ij))) iss8=10**(7-ij-1)
                end if                           
     ...
     +           partrue*10.d0**(-7+ij-1)
c simulate a new Benford number if the actual number of 
c                    significant digits is < 6 ==> go to label 12000 above     
!                isscheck=floor(ss*10.d0**4)
!                sscheck=ss-dfloat(isscheck*10.d0**(-4))
!                if(sscheck.lt.tol) then
!                  isscount=isscount+1
!                  goto 12000
!                end if 
...
c check on actual truncation/rounding before jittering
c             npar=6 digits x npartrue=1 parameter 
c             use option a) for approximation in countdigit
              call COUNTDIGIT(ss,nd,i,npar,idistr)                           
              avntrunc120(nd)=avntrunc120(nd)+1.d0/fn2
c check countdigit
              if(i.le.cumtrunc(1)) then
                if(nd.ne.1) write(*,*) i,'1 digit',nd,ss 
              else if(i.gt.cumtrunc(1).and.i.le.cumtrunc(2)) then
                if(nd.ne.2) write(*,*) i,'2 digits',nd,ss 
              else if(i.gt.cumtrunc(2).and.i.le.cumtrunc(3)) then
                if(nd.ne.3) write(*,*) i,'3 digits',nd,ss 
              else if(i.gt.cumtrunc(3).and.i.le.cumtrunc(4)) then
                if(nd.ne.4) write(*,*) i,'4 digits',nd,ss
              else if(i.gt.cumtrunc(4).and.i.le.cumtrunc(5)) then
                if(nd.ne.5) write(*,*) i,'5 digits',nd,ss 
              else if(i.gt.cumtrunc(5).and.i.le.cumtrunc(6)) then
                if(nd.ne.6) write(*,*) i,'6+ digits',nd,ss 
              end if
c Perform jittering + save significands
              rng=DRNUNF()
c more jittering also when n=200
              if(n.lt.200) then              
                s(i)=ss+rng*(10.d0**(-9))
...
c Save Significand s(i) in (0,1): fractional part of log10(|x|)
c (if dummy isavesig=1 and idistr=120)
              jout=100+j
              if(isavesig.eq.1) then
c more jittering also when n=200
                if(n.lt.200) then
...
c continue computations if idistr=121
13000         continue
c Skip the computation of s(i) for the other distributions
              goto 1001
            end if  
c
c idistr =122: NEW INDIVIDUAL Truncated Benford TRUNCNEWREPIND = Last-Digit(s) 
c              Dirac distribution with fixed digit 0 replicating individual
c              number of digits (read the number of digits *numdigit* and 
c              the repetition matrix *matrep* from file par_matrep.txt)
c              * npartrue=1 as with idistr=120 and idistr=121
c              * simulation loop similar to the loop with idistr=120
c              * use dummy vector *irep* for simulation of unique Benford
c                random numbers (difference wrt idistr=121) ==> if
c                irep(i)=1: the significand is repeated from another unit
c                (skip all computations: see label 13001 below) 
c              * numd=numdigit (read from file par_numdigitind.txt)  
c                refers to individual *unsorted* significands 
c                (difference wrt idistr=121)
c                N.B.: numd <= 6 ==> approximate to numd=6 if the true
c                      number of digits is > 6
c              * simulate a new Benford number to be truncated/rounded
c                if the actual number of significant digits is less than
c                the expected number ==> see label 12001 below
            if(idistr.eq.122) then
              numd=numdigit(i) 
              if(irep(i).eq.1) then
                avnrep=avnrep+1.d0/fn2
                goto 13001
              end if
12001         continue              
              rnb=DRNUNF()
     ...
     +           " in pow sim #",jj         
              ...
              ssb=10.d0**rnb
c Perform truncation/rounding of individual unsorted significands 
c (vector *numdigit* read on input)
              s(i)=ssb
              iss8=floor(s(i)*(10.d0**(numd-1)))
c Rounded Benford distribution if *itrunc=0*
c                NB: iss8=10**(numd-1)  if  iss8=10**numd  after rounding              
              if(itrunc.eq.0) then
                ss=s(i)*10.d0**(numd-1)-dfloat(iss8)
                if(ss.ge.0.5d0) iss8=iss8+1
                if(iss8.eq.(10**numd)) iss8=10**(numd-1)                  
              end if                                       
              s(i)=dfloat(iss8)*(10.d0**(-(numd-1)))
c simulate a new Benford number if the actual number of 
c                    significant digits is < numd ==> go to label 12001 above        
c use COUNTDIGIT for checking 
c             use option a) for approximation 
              ss=s(i)
              call COUNTDIGIT(ss,nd,i,npar,idistr)                           
...
c Check repetitions and replace repeated individual unsorted significands
c (matrix *matrep* read on input)
c Also update dummy *irep* ==> skip further simulation for repeated 
c individual unsorted significands (see goto statement and label 13001 below)
              do ii=i+1,n
                if(matrep(i,ii).eq.1) then
                  s(ii)=s(i)
                  irep(ii)=1
                end if
              end do
c continue computations if irep(i)=1 ==> repeated 
c             individual unsorted significand
13001         continue
              ss=s(i)
c check on actual truncation/rounding before jittering
c             npar=6 digits x npartrue=1 parameter 
c             use option a) for approximation in countdigit
              call COUNTDIGIT(ss,nd,i,npar,idistr)                           
              avntrunc122(nd)=avntrunc122(nd)+1.d0/fn2
c check countdigit
              if(nd.ne.numd) then
                write(*,*) i,ss,'  simulated # of digits =',nd,
     +           '   input # of digits =',numdigit(i)
                avnumd=avnumd+1.d0/fn2
              end if                
c Perform jittering + save significands
              rng=DRNUNF()
c more jittering also when n=200
...
c Save Significand s(i) in (0,1): fractional part of log10(|x|)
c (if dummy isavesig=1 and idistr=122) ==> increased output: also save numd
              jout=100+j
              if(isavesig.eq.1) then
c more jittering also when n=200
                if(n.lt.200) then
                  write(jout,304) ss,numd,rng*(10.d0**(-9)),
     +             s(i),dlog10(s(i))
c even more jittering for big n and idistr.eq.120
                else if(n.lt.500) then
                  write(jout,304) ss,numd,rng*(10.d0**(-7)),
     +             s(i),dlog10(s(i))
                else
!                  write(jout,304) ss,nd,rng*(10.d0**(-6)),
!     +             s(i),dlog10(s(i))                
                  write(jout,304) ss,numd,rng*(10.d0**(-5)),
     +             s(i),dlog10(s(i))                
                end if                
              end if
c Skip the computation of s(i) for the other distributions
...
1001        continue
c skip computations if idistr=121 ==> compute 
c             significand fractional part + digits outside the 
c             loop over sample elements
            if(idistr.ne.121) then
c COMPUTE sFR(i) = <s(i)> = significand *fractional part*: to be used 
c                           in tests KSFR and VKFR
              sFR(i)=s(i)-floor(s(i))
c
...
c
c TRUNCNEWREP distribution if idistr=121 ==> *sort* the significands
c             in *ascending order* 
c           + *repeat* sorted Benford values according to dummies *irep*
c             read from file par_rep.txt
c           + perform *truncation/rounding* of the sorted and repeated significands
c             according to the number of digits *numdigit* read from file
c             par_numdigit.txt
c           + perform jittering in the resulting significands to avoid ties
c             in DKSONE (see idistr=120)
c           + *save* the sorted + repeated + truncated/rounded + jittered
c             significands (as for idistr=120 but with different output/format) 
c             ==> do not save the original Benford value but save numd: see 
c             format 303 instead of format 302
c           + compute *significand fractional part* and *digits*
c             (see label 1001)
          if(idistr.eq.121) then
c
c Sort the significands 
c (in the current version do not return the corresponding permutation vector)
!            CALL DSVRGP (n, s, s, ipermdigit)
            CALL DSVRGN (n, s, s)
c
c Loop for repetitions (not yet truncated/rounded) of *sorted* significands
c according to the value of *irep*        
            do i=2,n
              if(irep(i).eq.1) s(i)=s(i-1)
            end do            
c
c Further loop of *sorted* significands (possibly not efficient ==> to be improved) 
c for: 
c * truncation/rounding according to the number of digits *numdigit* of 
c   the *sorted* significands 
c * jittering (see idistr=120) 
c * saving the sorted and repeated significands 
c * computing significand fractional parts and digits (see label 1001)
            do i=1,n
c Perform truncation/rounding of sorted significands (numdigit read on input)
              numd=numdigit(i)
              iss8=floor(s(i)*(10.d0**(numd-1)))
c Rounded Benford distribution if *itrunc=0*
c                NB: iss8=10**(numd-1)  if  iss8=10**numd  after rounding              
              ...
              s(i)=dfloat(iss8)*(10.d0**(-(numd-1)))
c simulate a new Benford number if the actual number of 
c                    significant digits is < nd ==> *To be fixed*  
c
c check on actual truncation/rounding before jittering
c             npar=6 digits x npartrue=1 parameter
c             use option a) for approximation in countdigit
              ss=s(i) 
              call COUNTDIGIT(ss,nd,i,npar,idistr)
              avntrunc121(nd)=avntrunc121(nd)+1.d0/fn2
c Perform jittering + save significands             
              rng=DRNUNF()
c more jittering also when n=200
              if(n.lt.200) then              
                s(i)=s(i)+rng*(10.d0**(-9))
c even more jittering for big n and idistr.eq.121           
              else if(n.lt.500) then 
                s(i)=s(i)+rng*(10.d0**(-7))
              else                
!                s(i)=s(i)+rng*(10.d0**(-6))
              ...
              if(s(i).ge.10.d0) then 
                write(*,*) "Error in jittering idistr=121:",j,jj,i,s(i)
                go to 9999
              end if
!c Save significands + observation #  + number of digits          
!              jout=100+j
!              if(isavesig.eq.1) then
!!                write(jout,303) rng,ipermdigit(i),numd,s(i),dlog10(s(i))
!                write(jout,303) rng,numd,s(i),dlog10(s(i))
!              end if 
c COMPUTE sFR(i) = <s(i)> = significand *fractional part*: to be used 
c                           in tests KSFR and VKFR
              sFR(i)=s(i)-floor(s(i))
c Save significands + observation #  + number of digits          
c save fractional part of the significand instead of rng and
c               dlog10(s(i)) ==> see new format 303 (also save some memory space)
              jout=100+j
              if(isavesig.eq.1) then
!                write(jout,303) rng,ipermdigit(i),numd,s(i),dlog10(s(i))
                write(jout,303) rng,numd,s(i),sFR(i)
              end if               
!c Check empirical mean and standard deviation of simulated data 
!              checkmean(j)=checkmean(j)+simdata(i)/(fn*fn2)
!              checkvar(j)=checkvar(j)+simdata(i)*simdata(i)/(fn*fn2)
...
C                                         new option: isavetest read from file
C also save asymptotic p-value for Kuiper tests VK + VKFR
C                     *see new format 43*
C                     increased precision for saving KS output 
C                     *see new format 44*
...
40        format(f12.6,1x,i2)
41        format(f12.6,2x,"ncomp",i4,1x,i2)
!44        format(6f12.6,6x,i3)
44        format(f12.10,2x,f12.10,2x,f12.10,f12.6,2x,f12.10,2x,f12.10,
     +     1x,i2)
43        format(f12.10,2x,f12.10,1x,i2)
...
c
c MAD test on first digit + asymptotic p-value for 
c XMADn = standardized MAD = (sqrt(n)*MAD - EMAD)/sqrt(VARMAD)
c     ==> EMAD and VARMAD are the moments of sqrt(n)*MAD
c     ==> one sided test as in Cerqueti and Lupi, TEST, 2023, Eq. (14)
c     ==> only save XMAD (not XMADn) + asymptotic p-value (see below)
c (could be incorporated into X2ONE)
          call XMADONE(jsimul,n,maxn,d1,benf1p,XMAD)
          XMADn=(dsqrt(fn)*XMAD-EMAD)/dsqrt(VARMAD)
          pvalXMADas=1.d0-dnordf(XMADn)
c
...
!            write(530,40) VK,j
            write(530,43) VK,pvalVKas,j
            write(540,44) (KSFR(jjjj),jjjj=1,6),j
!            write(550,40) VKFR,j
            write(550,43) VKFR,pvalVKFRas,j
            write(560,43) XMAD,pvalXMADas,j
          end if
c
c
c skip the loop for computing exact p-values of combined 
...
c computed ==> not (yet) used in combined tests 
c
          ...
          do ij=1,nsim1
            ...
            read(146,30) XMADss
            if(dfloat(XMADss).gt.XMAD) then
              pvalXMAD=pvalXMAD+1.0/float(nsim1)
              istopXMAD=0
            end if                  
...
1002      continue
          ...
          CLOSE(146)
c save exact p-values for the tests to be combined
          if(isavetest.eq.1) then
            write(201,201) pvalX2,pvalQ,pvalKS,pvalX2DIF,pvalX2DIF2T,
     +       pvalSCOREBIC,pvalXLRTBIC,pvalVK,pvalKSFR,pvalVKFR,pvalXMAD
201   format(11f13.10)     
          end if          
...
c Individual tests          
     ...
     +       powas(j,jjj,15)+1.d0/fn2
            if(XMAD.gt.pind(jjj,16)) powind(j,jjj,16)=powind(j,jjj,16)+
     +       1.d0/fn2
!            if(pvalXMADas.lt.testsize2(jjj)) powas(j,jjj,16)=
!     +       powas(j,jjj,16)+1.d0/fn2
            if(XMAD.gt.percas(jjj,16)) powas(j,jjj,16)=powas(j,jjj,16)+
     +       1.d0/fn2
c Combined test X2-Q
            pvalmin=pvalX2
...
c increased precision for saving p-values 
c                     reduce output to save space in files
c                     ==> see new formats 50-55
c
...
c are computed in the loop for combined tests 
...
c
c Possibly write test statistics after computation ==> see version
c Streamflow_NOmatrep
c
          if(idistr.lt.0) then          
...
c            write(61,50) XLRT2,pvalXLRT2
            ...
            write(66,55) XMAD,pvalXMADas,pvalXMAD  
          end if
...
50        format(f12.6,4x,f12.10)
55        format(f12.6,4x,f12.10,4x,f12.10)
54        format(f12.10,4x,f12.10,4x,f12.10,f12.6,4x,f12.10,4x,f12.10,
     +     4x,f12.10)
51        format(f12.6)
52        format(f12.6,4x,i4,4x,f12.10)
c
C
...
9912    format(1x,"MULTIPLE DATA SETS: nsim2 = ",i9,3x,"(see output ",
     +   "files for individual statistics and p-values)")
        ...
        if(idistr.eq.(-1)) then
          ...
          write(99,9911) nsim2
          write(99,*)
          write(99,9977) numreptot,nunique
9977      format(1x,'Total # of REPLICATED values: numreptot =',i12,/,
     +     1x,'Total # of UNIQUE values: nunique =',i10)
        else
          write(99,*)
          write(99,9912) nsim2
        end if
...
99381   format(1x,"VK on the significand *fractional part* =",f12.6,10x,
     +   "Asymptotic p-value =",f9.6,6x,"Exact p-value =",f9.6)     
        write(99,99382)XMAD,pvalXMADas,pvalXMAD
99382   format(1x,"One-sided MAD test on the first digit =",f12.6,10x,
     +   "Asymptotic p-value =",f9.6,6x,"Exact p-value =",f9.6)     
        write(99,9948)
...
912     format(1x,"POWER OF TESTS - EXACT + ASYMPTOTIC",//,1x,"# sim =",
     ...
     + "TIES IN KS TEST (more jittering when n>=200 and also n>500)",//,
     ...
     +   1x,"# sim for null quantile estimation",i12)
          ...
          write(9,9122) nsim2,n,npartrue,idistr,par(1),nsim1
9122    format(1x,"POWER OF TESTS - EXACT + ASYMPTOTIC",//,1x,"# sim =",
     +   i9,
     +   8x,"n =",i6,8x,"# param =",i3,8x,"Distribution # =",i3,//,
     + 25x,"* ROUNDED BENFORD ==> LAST-DIGIT ROUNDED DISTRIBUTION: *",/,
     + 25x,"  + JITTERING IN THE 10th DECIMAL PLACE TO AVOID ",
     ...
     +   1x,"# sim for null quantile estimation",i12)
        ...
        write(9,*)
        write(9,9400)
9400    format(1x,'Check on actual truncation/rounding of digits ',
     +   'before jittering')        
        do j=1,npartrue
          sumntrunc12=0.d0
          do jj=1,npar
            sumntrunc12=sumntrunc12+avntrunc12(jj,j)
          end do
          write(9,9010) j
          write(9,9011) (avntrunc12(jj,j),jj=1,npar),sumntrunc12
9011      format(6f15.5,10x,'check on n =',f15.5)          
        end do
      end if
c
c Rounded Benford distribution if *itrunc=0* 
      if(idistr.eq.120) then
        if(itrunc.eq.1) then
          ...
          OPEN(unit=9,file='outROUNDNEW.txt')
        end if 
        if(itrunc.eq.1) then       
...
9120    format(1x,"POWER OF TESTS - EXACT + ASYMPTOTIC",//,1x,"# sim =",
     ...
     + 25x,"  + JITTERING IN THE 10th DECIMAL PLACE TO AVOID ",
     +  "TIES IN KS TEST (more jittering when n>=200)",//,
     + 25x,"* CHECK ON THE ACTUAL NUMBER OF SIGNIFICANT DIGITS *",//,
     ...
     +   1x,"# sim for null quantile estimation",i12)
          ...
          write(9,9121) nsim2,n,npartrue,idistr,(j,j=1,npar),
     +    (par(j),j=1,npar),(ntrunc(j),j=1,npar),(cumtrunc(j),j=1,npar),
     +    nsim1
9121    format(1x,"POWER OF TESTS - EXACT + ASYMPTOTIC",//,1x,"# sim =",
     +   i9,
     +   8x,"n =",i6,8x,"# param =",i3,8x,"Distribution # =",i3,//,
     +25x,"*NEW ROUNDED BENFORD* ==> DISTRIBUTION WITH LAST DIGIT(S)",
     + " ROUNDED IN FIXED PROPORTIONS OF CASES (TO BE READ AS INPUT)",/,
     + 25x,"  + JITTERING IN THE 10th DECIMAL PLACE TO AVOID ",
     +  "TIES IN KS TEST (more jittering when n>=200)",//,
     +   1x,"# of significant digits j = ",9x,6i18,//,
     +   1x,"Proportion with j significant digits",3x,6f18.3,//, 
     ...
     +   1x,"# sim for null quantile estimation",i12)     
        ...
        write(9,9123) isscount,
     +   dfloat(isscount)/(dfloat(n)*dfloat(nsim2))
9123  format(1x,"Number of multiple simulated Benford random numbers",
     + i20,/,
     + 1x,"Proportion of multiple simulated Benford random numbers",
     + f12.9)      
        write(9,*)
        write(9,9200) 
        do j=1,npartrue
        ...
        do j=1,npartrue
          sumntrunc120=0.d0
          do jj=1,npar
            sumntrunc120=sumntrunc120+avntrunc120(jj)
          end do
          write(9,9010) j
          write(9,9011) (avntrunc120(jj),jj=1,npar),sumntrunc120      
        end do        
      end if      
c      
c TRUNCNEWREP distribution  
      if(idistr.eq.121) then
        avndist=fn-avnrep      
        if(itrunc.eq.1) then
          OPEN(unit=9,file='outTRUNCNEWREP.txt')
        else
          OPEN(unit=9,file='outROUNDNEWREP.txt')
        end if 
        if(itrunc.eq.1) then       
          write(9,9125) nsim2,n,npartrue,idistr,(j,j=1,npar),
!     +    (par(j),j=1,npar),
     +    (ntrunc(j),j=1,npar),(cumtrunc(j),j=1,npar),
     +    avnrep,avndist,nsim1
9125    format(1x,"POWER OF TESTS - EXACT + ASYMPTOTIC",//,1x,"# sim =",
     +   i9,
     +   8x,"n =",i6,8x,"# param =",i3,8x,"Distribution # =",i3,//,
     + 25x,"*NEW  *TRUNCATED*  BENFORD WITH   REPETITIONS*:",/,
     + 25x,"DISTRIBUTION WITH LAST DIGIT(S) 0 IN FIXED PROPORTIONS OF ",
     + "CASES (TO BE READ AS INPUT)",/,
     + 25x,"  + REPEATED BENFORD VALUES IN THE *SORTED* SIGNIFICANDS",
     + "(TO BE READ AS INPUT ==> see file *par_rep.txt*)",/,
     + 25x,"  + JITTERING IN THE 10th DECIMAL PLACE TO AVOID ",
     +  "TIES IN KS TEST (more jittering when n>=200)",//,
     + 25x,"* CHECK ON THE ACTUAL NUMBER OF SIGNIFICANT DIGITS *",//,
     +   1x,"# of significant digits j = ",9x,6i18,//,
!     +   1x,"Proportion with j significant digits",3x,6f18.3,//, 
     +   1x,"# of cases with j significant digits",1x,6i18,//, 
     +   1x,"Cumulative # of cases",16x,6i18,//, 
     +   1x,"average # of *repeated sorted* significands = ",f12.6,//
     +   1x,"average # of *distinct sorted* significands = ",f12.6,//
     +   1x,"# sim for null quantile estimation",i12)
        else
          write(9,9126) nsim2,n,npartrue,idistr,(j,j=1,npar),
     +    (par(j),j=1,npar),(ntrunc(j),j=1,npar),(cumtrunc(j),j=1,npar),
     +    avnrep,avndist,nsim1
9126    format(1x,"POWER OF TESTS - EXACT + ASYMPTOTIC",//,1x,"# sim =",
     +   i9,
     +   8x,"n =",i6,8x,"# param =",i3,8x,"Distribution # =",i3,//,
     + 25x,"*NEW  *ROUNDED*  BENFORD WITH   REPETITIONS*:",/,
     + 25x,"DISTRIBUTION WITH LAST DIGIT(S) *ROUNDED* IN FIXED ",
     + "PROPORTIONS OF CASES (TO BE READ AS INPUT)",/,
     + 25x,"  + REPEATED BENFORD VALUES IN THE *SORTED* SIGNIFICANDS",
     + "(TO BE READ AS INPUT ==> see file *par_rep.txt*)",/,
     + 25x,"  + JITTERING IN THE 10th DECIMAL PLACE TO AVOID ",
     +  "TIES IN KS TEST (more jittering when n>=200)",//,
     ...
     +   dfloat(isscount)/(dfloat(n)*dfloat(nsim2))     
        ...
        do j=1,npartrue
          sumntrunc121=0.d0
          do jj=1,npar
            sumntrunc121=sumntrunc121+avntrunc121(jj)
          end do        
          write(9,9010) j
          write(9,9011) (avntrunc121(jj),jj=1,npar),sumntrunc121     
        end do              
      end if      
c
c TRUNCNEWREPIND distribution  
      if(idistr.eq.122) then
        if(itrunc.eq.1) then
          OPEN(unit=9,file='outTRUNCNEWREPIND.txt')
        else
          OPEN(unit=9,file='outROUNDNEWREPIND.txt')
        end if 
        avndist=fn-avnrep        
        if(itrunc.eq.1) then       
          write(9,9135) nsim2,n,npartrue,idistr,(j,j=1,npar),
!     +    (par(j),j=1,npar),
     +    (ntrunc(j),j=1,npar),(cumtrunc(j),j=1,npar),
     +    avnrep,avndist,avnumd,nsim1
9135    format(1x,"POWER OF TESTS - EXACT + ASYMPTOTIC",//,1x,"# sim =",
     +   i9,
     +   8x,"n =",i6,8x,"# param =",i3,8x,"Distribution # =",i3,//,
     + 25x,"*NEW  *TRUNCATED*  BENFORD WITH INDIVIDUAL REPETITIONS*:",/,
     + 25x,"DISTRIBUTION WITH LAST DIGIT(S) 0  AS IN *SAMPLE DATA*",/,
     + 25x,"+ REPEATED BENFORD VALUES IN THE *UNSORTED* SIGNIFICANDS",/,
     + 25x,"(TO BE READ AS INPUT ==> see repetition matrix *matrep*",
     + " in file *par_matrep.txt*)",/,
     + 25x,"  + JITTERING IN THE 10th DECIMAL PLACE TO AVOID ",
     +  "TIES IN KS TEST (more jittering when n>=200)",//,
     + 25x,"* CHECK ON THE ACTUAL NUMBER OF SIGNIFICANT DIGITS *",//,
     +   1x,"# of significant digits j = ",9x,6i18,//,
!     +   1x,"Proportion with j significant digits",3x,6f18.3,//, 
     +   1x,"# of cases with j significant digits",1x,6i18,//, 
     +   1x,"Cumulative # of cases",16x,6i18,//, 
     +   1x,"average # of *repeated unsorted* significands = ",f12.6,//,
     +   1x,"average # of *distinct unsorted* significands = ",f12.6,//,
     +   1x,"average # of significands for which *nd.ne.numd ",f12.6,//,          
     +   1x,"# sim for null quantile estimation",i12)
        else
          write(9,9136) nsim2,n,npartrue,idistr,(j,j=1,npar),
     +    (par(j),j=1,npar),(ntrunc(j),j=1,npar),(cumtrunc(j),j=1,npar),
     +    avnrep,avndist,avnumd,nsim1
9136    format(1x,"POWER OF TESTS - EXACT + ASYMPTOTIC",//,1x,"# sim =",
     +   i9,
     +   8x,"n =",i6,8x,"# param =",i3,8x,"Distribution # =",i3,//,
     + 25x,"*NEW  *ROUNDED*  BENFORD WITH  INDIVIDUAL REPETITIONS*:",/,
     + 25x,"DISTRIBUTION WITH LAST DIGIT(S) *ROUNDED* AS IN ",
     + " *SAMPLE DATA*",
     + 25x,"+ REPEATED BENFORD VALUES IN THE *UNSORTED* SIGNIFICANDS",/,
     + 25x,"(TO BE READ AS INPUT ==> see repetition matrix *matrep*",
     + " in file *par_matrep.txt*)",/,
     ...
     +   dfloat(isscount)/(dfloat(n)*dfloat(nsim2))     
        ...
        do j=1,npartrue
          sumntrunc122=0.d0
          do jj=1,npar
            sumntrunc122=sumntrunc122+avntrunc122(jj)
          end do        
          write(9,9010) j
          write(9,9011) (avntrunc122(jj),jj=1,npar),sumntrunc122     
        end do              
      end if 
c      
      if(idistr.eq.13) then
...
888      format(20x,"EXACT TESTS  -  * Combined tests are performed *
     + :  itest =",i3)
        if(itest.lt.0) then
         write(9,*) 
         write(9,8881)
8881     format(20x,"NB: READ SIMULATED STATISTICS FROM PREVIOUS RUNS 
     +   ==>   see FILES  sim*.txt")        
        end if
      end if
      write(9,*)
...
8789  format(1x,"BIC selection of # of components for *LR TEST*")
      ...
      write(9,*)
      write(9,*)
      write(9,7888) EMAD,dsqrt(VARMAD)
7888  format(1x,"Asympt. Expected value of sqrt(n)*MAD: EMAD =",f12.8,
     + 6x,"Asympt. Std. Dev. of sqrt(n)*MAD: sqrt(VARMAD) =",f12.8)
      write(9,7889) EMADn,dsqrt(VARMADn)
7889  format(1x,"Asympt. Expected value of MAD: EMADn =",f12.8,11x,
     + "Asympt. Std. Dev. of MAD: sqrt(VARMADn) =",f12.8)    
      write(9,*)
      write(9,*)
      write(9,*)
      write(9,*)
     ...
     +           "selection of components"
      ...
      write(9,*)" 14:  KSFR   = Kolmogorov-Smirnov two-sided test on ",
     + "the significand *fractional part*"
      write(9,*)" 15:  VKFR   = Kuiper test on the significand ",
     + "*fractional part*"      
      write(9,*)" 16:  XMAD   = MAD test on the first digit (one-sided)"      
      write(9,*)
      ...
      write(9,*)" 9:   X2DIFKSFR  = Combined X2DIF-KS test on the ",
     + "significand fractional part (to be used when X2 accepts)"
      write(9,*)" 10:  X2DIFVKFR  = Combined X2DIF-Kuiper test on the ",
     + "significand fractional part (to be used when X2 accepts)"
      write(9,*)
     ...
     *           " df = 4" 
     ...
     +           "selection of components df = 2"
      ...
      write(9,*)" 14: KSFR   = p-value from routine DKSONE on the ",
     + "significand *fractional part*" 
      write(9,*)" 15: VKFR   = Kuiper test (see references) on the ",
     + "significand *fractional part*"
      write(9,*)" 16: XMAD   = MAD test on the first digit (one-sided)"      
      write(9,*)
      write(9,*)
...
C *************************************************************
C
      SUBROUTINE XMADONE(jsimul,n,maxn,d1,benf1p,XMAD)
C COMPUTE MAD ON FIRST DIGIT
      IMPLICIT NONE
      integer*4 i,j,jj,jj1,jsimul,n,maxn,d1(maxn),d1freq(9)
      real*8 benf1p(9),XMAD,xx,fn
c compute first-digit frequencies (as in X2ONE)
      do j=1,9
        d1freq(j)=0
      end do
      do i=1,n
        jj1=d1(i)
        if(jj1.lt.1.or.jj1.gt.9) then
          write(*,*) "XMADONE: error in 1st digit of sample unit",i,
     +    " d1 =",jj1
          pause
        end if
        d1freq(jj1)=d1freq(jj1)+1
...
c compute MAD
      ...
      fn=dfloat(n)       
      do j=1,9
        xx = dfloat(d1freq(j))/fn-benf1p(j)
        XMAD = XMAD + dabs(xx)
      end do
      XMAD=XMAD/9.d0
      RETURN
...
C COMPUTE HOTELLING SUM-INVARIANCE TEST Q
      IMPLICIT NONE
...
C RETURNS THE DISTRIBUTION FUNCTION OF 
C         U = FRACTIONAL PART OF S(X)   
C for S(X) in [1,10)  (see vector sFR)  and   U in [0,1)
C UNDER BENFORD'S LAW
C                                          
      IMPLICIT NONE
...
C DISTRIBUTION - KANTER ALGORITHM AS ADAPTED BY LUCIO BARABESI
...
C *******************************************************************
C
      SUBROUTINE COUNTDIGIT(sd,nd,i,npar,idistr)
C RETURNS THE NUMBER OF SIGNIFICANT DIGITS nd 
C OF INPUT VALUE sd of unit i
C
C Two options:
C a) Perform computation in single precision: 
C    * CURRENT OPTION IN SIMULATION *  ==>  idistr>=0
C b) write/read the (expanded) significand to file (to be checked/improved):
C     * CURRENT OPTION IN DATA ANALYIS *  ==>  idistr<0
C 
C (Computation up to ndmax=npar=6 digits: nd=6 ==> nd>=6)
C
      IMPLICIT NONE
      real*8 sd,sx,sx8,ss,ss8
      real*8 tol
      real*4 sx4,ss4
      integer*4 nd,ndmax,npar,idistr,iss,j,i
      integer*4 iss4
      integer*8 iss8
c
c Set and check the max number of significant digits
      if(npar.ne.6) then
        write(*,*) 'WARNING: npar=/6 ==> check max number of ',
     +   'significant digits'
        nd=6
        return
      end if    
      ndmax=npar   
c      
c Compute significand sx ==> repeat computation of main program: to be improved 
!
!c numerical check
!      sd=2.0000000777d0
!      sd=2.0d0
...
! 
      ...
      sx=10.d0**ss 
c      
c Expand the significand up to 5 digits for the integer part (ndmax=npar=6)
c and check the fractional part ==> if 0 repeat
c Two options:
c a) Perform computation in single precision: fractional parts of the order
c    of 10^(-8) are set to 0 after expansion ==> e.g. (see above)
c    * nd=1 for significands like sx=2.0000000777*
c b) Round the (expanded) significand to 9 significant digits (see format 1)
c    ==> write/read the (expanded) significand to file: to be improved
c  * Simulation: option a) *
c  * Data Analysis: option b) *
1     format(f30.10)
      tol=10.d-9
      do j=1,ndmax
        nd=ndmax-j+1
        sx8=sx*10.d0**(ndmax-1-j)  
c Option b): to be checked/improved
        if(idistr.lt.0) then
          open(unit=1,file='Sxround.txt')  
          write(1,1) sx8
          close(1)
          open(unit=1,file='Sxround.txt')  
          read(1,1) sx8     
          ...
          iss8=floor(sx8)
          ss8=sx8-(dfloat(iss8))
          if(ss8.gt.tol) return 
        else
c Option a)
          sx4=real(sx8)        
          iss4=floor(sx4)
          ss4=sx4-(float(iss4))
          if(ss4.ne.0.0) return               
        end if
c     
!c start numerical check
!          write(*,*)
!          write(*,*) 'unit',i
!          write(*,*) 'input data + floor',sd,floor(sd)
!          write(*,*) 'Sx + floor',sx,floor(sx)
!          write(*,*)
!          write(*,*) 'single precision',sx4,iss4,ss4,nd  
!          write(*,*) 
!          iss8=floor(sx8)
!          ss8=sx8-(dfloat(iss8))     
!          nd=ndmax-j+1
!          write(*,*) 'dble precision',sx8,iss8,ss8,nd
!          write(*,*) 
!          ss8=sx8-(dint(sx8))
!          iss8=idint(dint(sx8))     
!          nd=ndmax-j+1
!          write(*,*) 'dble precision 2',sx8,iss8,ss8,nd
!          write(*,*)           
!          sx8=sx*10.d0**(ndmax-1-j)      
!          open(unit=1,file='Sxround.txt')  
!          write(1,1) sx8
...
!          ss8=sx8-(dfloat(iss8))     
!          nd=ndmax-j+1
!          write(*,*) 'dble precision - WRITE + ROUND',sx8,iss8,ss8,nd
!          write(*,*)         
!!                  
!          sx8=sd
!          sx8=sx8+(10.d-15)-(10.d-15)         
!          iss8=nint(floor(sx8*10.d0)/10.d0)          
!          ss8=sx8-(dfloat(iss8))     
!          nd=ndmax-j+1
!          write(*,*) 'Dome precision: input',sd,sx8,iss8,ss8,nd
!          write(*,*)
!!          
!          sx8=sx
!          sx8=sx8+(10.d-15)-(10.d-15)
...
!          ss8=sx8-(dfloat(iss8))     
!          nd=ndmax-j+1
!          write(*,*) 'Dome precision: Sx',sx,sx8,iss8,ss8,nd
!          write(*,*)
!!          
!          sx8=sx*10.d0**(ndmax-1-j)
!          sx8=sx8+(10.d-15)-(10.d-15)
...
!          ss8=sx8-(dfloat(iss8))     
!          nd=ndmax-j+1
!          write(*,*) 'Dome precision: Sx*10^(5-j)',sx,sx8,iss8,ss8,nd
!          pause
!c end numerical check
      end do
c
      RETURN
      END
